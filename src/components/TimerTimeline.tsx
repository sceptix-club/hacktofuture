...
function Timeline() {
  ...
  cardRefs.current = Array(cards.length)
    .fill(null)
    .map((_) => useRef());

  const rowCount = Array(cards.length)
    .fill(null)
    .map((_, i) => Math.ceil(cards[i].length / 2));
  const maxRow = rowCount.slice().sort((a, b) => b - a)[0];

  const setZIndex = (curr) => {
    cardRefs.current.forEach((card, i) => {
      if (i < curr) {
        card.style.zIndex = curr - i;
      } else {
  ...
  const cardClicked = (event) => {
    const xBounds = cardRefs.current[currentCard].getBoundingClientRect().left;
    const x = event.clientX - xBounds;
  ...
  return (
    <div className="timeline">
      <div
        ...
        style={{
          display: "grid",
          gridTemplateColumns: `repeat(${maxRow}, 1fr)`,
        }}
      >
        {times.map((time) => (
